...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<?>> handleUserNotFoundException(UserNotFoundException e) {
        return ResponseEntity.status(404).body(ApiResponse.createError(e.getMessage()));
    }

    // 인증 실패
    @ExceptionHandler(AuthFailException.class)
    public ResponseEntity<ApiResponse<?>> handleAuthFailException(AuthFailException e) {
        return ResponseEntity.status(401).body(ApiResponse.createError(e.getMessage()));
    }
    ...
    public ResponseEntity<ApiResponse<?>> handleExtensionErrorException(ExtensionErrorException e) {
        return ResponseEntity.status(415).body(ApiResponse.createError(e.getMessage()));
    }
    ...
    public ResponseEntity<ApiResponse<?>> handleFailCreateTeamException(FailCreateTeamException e) {
        return ResponseEntity.status(500).body(ApiResponse.createError(e.getMessage()));
    }
}
